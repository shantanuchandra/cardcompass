...
**Status: conditional local-review build; not release-ready.** The final review's six Important findings and three minor evidence/copy gaps are repaired in three implementation commits:
...
| `flutter test` | **Not green:** 313 passed, 4 skipped, 11 failed. All 11 remaining failures are the recorded environment-bound Supabase cases: the local static endpoint is not PostgREST and VM storage/PKCE plugins or live credentials are unavailable. No UI test remains failed. |
| `flutter test test/core test/features test/shared test/widget_test.dart` | 312 passed, 0 failed. |
| Focused final-review Flutter selection (theme, auth, dashboard, cards, transactions, movie deals, settings) | 135 passed, 0 failed. |
...
| Chrome-target OAuth tests | 4 passed. `app_shell_brand_test.dart` still cannot load on Chrome because the test itself reads source with `dart:io File` (`Unsupported operation: _Namespace`); its VM run passes in the 312-test suite. |
...
The final focused selection was expanded with the theme contracts and add-card route integration test and exited 0 with 135 passed. Its core feature list remains:
...
| Dashboard | populated hierarchy/scale, cardless, redacted dashboard/Gmail errors, actions; card control: `dashboard card is a labeled keyboard-sized button`; bank resolution: `bank resolution redacts failures, clears stale errors, and retries` | populated, empty, errors, actions, bank/card controls, retry, and 200% fixture pass; loading **NOT VERIFIED** | N/A — widget fixture; no live-auth browser screenshot |
...
| Add Card | progress/validation/error/200%; keyboard result target; out-of-order search, short-query clear, dispose, save-dispose, and pop-failure race tests; route integration covers Back/save | listed initial, loading, error, async-race, keyboard, Back/save, and 200% states pass | N/A — widget fixture; no live-auth browser screenshot |
