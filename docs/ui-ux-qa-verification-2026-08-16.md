...
| `node --test test/landing/public-reading-browser.test.js` | 1 rendered Chrome test passed: 9 public HTML routes × 390/1440, real Tab traversal with a visible focus indicator, and `document.documentElement.scrollWidth === window.innerWidth`; under reduced-motion emulation, landing rotation stayed on Groceries for 4.25 seconds (beyond its 4-second interval), a real pointer click still selected Dining and synchronously updated the receipt, and computed receipt/button transition plus reveal animation durations were suppressed to 0.01ms. |
...
|---|---:|---|---|---|---|---|---|
| `/` | 390×844 | default | live Chrome: first and second Tab targets advanced; focused target visible with indicator | reduced motion live: auto-rotation static beyond 4s, manual Dining selection works, computed motion durations 0.01ms; 200% text: **NOT VERIFIED** | automated none (`390/390`) | Waitlist rejects blank email/consent, then reports configuration unavailable for synthetic valid data without transmission | `home-390x844.png` |
| `/` | 1440×900 | default | live Chrome: first and second Tab targets advanced; focused target visible with indicator | reduced motion live: media/root behavior plus landing rotation/manual-selection assertion; 200% text: **NOT VERIFIED** | automated none (`1440/1440`) | Scenario controls and waitlist contracts pass | `home-1440x900.png` |
...
- The same rendered test emulates `prefers-reduced-motion: reduce` and asserts the media query matches and the computed root scroll behavior becomes `auto` on every public HTML route at both viewports. On the landing page it also observes the active scenario for 4.25 seconds (longer than the 4-second rotation interval), verifies Groceries remains selected, dispatches a real pointer click to Dining and verifies the receipt updates, and checks representative computed transition/animation durations are 0.01ms. Flutter reduced motion remains fixture-only (`disableAnimations` tests), not live-browser verified.
