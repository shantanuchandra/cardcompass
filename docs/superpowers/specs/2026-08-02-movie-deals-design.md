...
- **`explicit`** — the rule's `partners` set is non-empty and contains the user's searched platform (case-insensitive), **or** the user searched with no platform selected and the rule's `partners` set is non-empty (any recorded partner is a positive signal regardless of which one the user asked about).
- **`communityConfirmed`** — the rule's `partners` set is empty, but ≥1 confirmed user report exists for the searched platform (§6) — under "Any Platform," this means ≥1 confirmation exists for **any** platform.
- **`unconfirmed`** — the rule's `partners` set is empty, no confirmations exist.
- **`notRequested`** — **§1.2 correction (this pass):** the user searched with no platform selected ("Any Platform") **and** the rule's `partners` set is empty. An earlier revision of this tier collapsed this case into `explicit` — reasoning that "there's nothing to be unconfirmed against" when the user didn't ask a narrowing question. That's backwards: choosing not to constrain the search is a user preference, not evidence that a rule with zero recorded partner data is platform-agnostic. Treating it as `explicit` let a rule with genuinely no partner information sail through the guaranteed-tier gate (below) purely because the search happened to be unconstrained, which is exactly the "system doesn't actually know if this applies" case the rest of §5 is built to prevent. `notRequested` is deliberately distinct from `unconfirmed`/`communityConfirmed`: it does not imply anyone tried and failed (or partially confirmed) to establish the platform — the question was simply never asked. It does not qualify for the guaranteed tier (below).

A rule with an empty `partners` set still surfaces as a candidate under any search (it may be a genuine platform-agnostic discount, or simply unrecorded) but is never presented as an unqualified match — the UI shows a caveat chip ("Platform not confirmed for this offer") for anything below `explicit`, including `notRequested`.
