...
| Direct web paths | Pass | A real HTTP server test serves `/app/admin2`, nested Admin2 paths, and queries from the Flutter index; redirects `/app` canonically and the legacy catalog URL exactly; and keeps missing/lookalike assets, API-like paths, and encoded traversal out of SPA fallback. A separate test-only authenticated/admin bootstrap was compiled as a release build and loaded in headless Chrome. Every navigation waited for a new document plus settled Flutter semantics: protected unauthenticated routes ended at `/app/login`; authenticated Admin2 showed Action Inbox; Cards and Settings showed their unique headings; and the legacy URL settled on Card Data with the exact section query. Every path had one `/app/` prefix and no route-not-found semantics. The normal production build and production source roots were scanned and contain no fixture marker, fake, or query seam. Hash routes are not part of the PathUrlStrategy contract. |
