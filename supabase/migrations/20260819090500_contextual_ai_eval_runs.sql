create table public.ai_eval_runs (
  ...
  status text not null default 'queued' check (status in ('queued','running','completed','completed_with_failures','failed','cancelled')), retry_generation integer not null default 0 check (retry_generation >= 0),
...
create table public.ai_eval_results (
  ...
  attempt_count integer not null default 1 check (attempt_count > 0), attempt_generation integer not null default 0 check (attempt_generation >= 0),
...
language plpgsql security definer set search_path = '' as $$ declare run public.ai_eval_runs; token uuid; cases jsonb; available integer; begin
  ...
  select count(*) into available from jsonb_array_elements(run.case_manifest) m where not exists(select 1 from public.ai_eval_results r where r.run_id=run.id and r.case_id=(m->>'case_id')::uuid and r.case_revision=(m->>'revision')::integer and (r.execution_status='succeeded' or r.attempt_generation>=run.retry_generation));
  ...
  select coalesce(jsonb_agg(m order by m->>'case_id',m->>'revision'),'[]'::jsonb) into cases from (select m from jsonb_array_elements(run.case_manifest) m where not exists(select 1 from public.ai_eval_results r where r.run_id=run.id and r.case_id=(m->>'case_id')::uuid and r.case_revision=(m->>'revision')::integer and (r.execution_status='succeeded' or r.attempt_generation>=run.retry_generation)) order by m->>'case_id',m->>'revision' limit least(_batch_limit, 5)) q;
...
language plpgsql security definer set search_path = '' as $$ declare run public.ai_eval_runs; prior public.ai_eval_results; recorded public.ai_eval_results; manifest_case jsonb; begin
  ...
  insert into public.ai_eval_results(run_id,case_id,case_revision,feature_key,baseline_output,candidate_output,deterministic_assertions,judge_verdict,requires_review,regression,severe_regression,baseline_latency_ms,candidate_latency_ms,baseline_input_tokens,baseline_output_tokens,candidate_input_tokens,candidate_output_tokens,estimated_cost_usd,execution_status,safe_failure_category,attempt_count,attempt_generation,claim_token)
  values(_run_id,_case_id,_case_revision,_result->>'feature_key',_result->'baseline_output',_result->'candidate_output',_result->'deterministic_assertions',_result->'judge_verdict',(_result->>'requires_review')::boolean,coalesce((_result->>'regression')::boolean,false),coalesce((_result->>'severe_regression')::boolean,false),coalesce((_result->>'baseline_latency_ms')::integer,0),coalesce((_result->>'candidate_latency_ms')::integer,0),coalesce((_result->>'baseline_input_tokens')::integer,0),coalesce((_result->>'baseline_output_tokens')::integer,0),coalesce((_result->>'candidate_input_tokens')::integer,0),coalesce((_result->>'candidate_output_tokens')::integer,0),coalesce((_result->>'estimated_cost_usd')::numeric,0),_result->>'execution_status',nullif(_result->>'safe_failure_category',''),coalesce(prior.attempt_count,0)+1,run.retry_generation,_lease_token)
  on conflict(run_id,case_id,case_revision) do update set baseline_output=excluded.baseline_output,candidate_output=excluded.candidate_output,deterministic_assertions=excluded.deterministic_assertions,judge_verdict=excluded.judge_verdict,requires_review=excluded.requires_review,regression=excluded.regression,severe_regression=excluded.severe_regression,baseline_latency_ms=public.ai_eval_results.baseline_latency_ms+excluded.baseline_latency_ms,candidate_latency_ms=public.ai_eval_results.candidate_latency_ms+excluded.candidate_latency_ms,baseline_input_tokens=public.ai_eval_results.baseline_input_tokens+excluded.baseline_input_tokens,baseline_output_tokens=public.ai_eval_results.baseline_output_tokens+excluded.baseline_output_tokens,candidate_input_tokens=public.ai_eval_results.candidate_input_tokens+excluded.candidate_input_tokens,candidate_output_tokens=public.ai_eval_results.candidate_output_tokens+excluded.candidate_output_tokens,estimated_cost_usd=public.ai_eval_results.estimated_cost_usd+excluded.estimated_cost_usd,execution_status=excluded.execution_status,safe_failure_category=excluded.safe_failure_category,attempt_count=excluded.attempt_count,attempt_generation=excluded.attempt_generation,claim_token=excluded.claim_token,updated_at=now() where public.ai_eval_results.execution_status='failed'
...
language plpgsql security definer set search_path = '' as $$ declare run public.ai_eval_runs; total integer; succeeded integer; failed integer; cost numeric; tokens jsonb; next_status text; remaining boolean; begin
...
language plpgsql security definer set search_path = '' as $$ declare run public.ai_eval_runs; continuation_required boolean; begin
