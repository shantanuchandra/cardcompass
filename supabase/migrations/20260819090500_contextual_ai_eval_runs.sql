create table public.ai_eval_runs (
  ...
  cost_ceiling_usd numeric(12,6) not null check (cost_ceiling_usd > 0), latency_ceiling_ms integer not null check (latency_ceiling_ms > 0),
  aggregate_metrics jsonb not null default '{}'::jsonb, token_usage jsonb not null default '{}'::jsonb,
  estimated_cost_usd numeric(12,6) not null default 0 check (estimated_cost_usd >= 0), safe_failure_category text check (safe_failure_category is null or safe_failure_category in ('cost_ceiling_reached','model_unavailable','invalid_model_output','provider_failed','persistence_failed')),
...
create table public.ai_eval_results (
  ...
  safe_failure_category text check (safe_failure_category is null or safe_failure_category in ('model_unavailable','invalid_model_output','provider_failed','persistence_failed')),
...
grant select, insert, update on public.ai_eval_runs to service_role;
grant select, insert, update on public.ai_eval_results to service_role;
...
language plpgsql set search_path = '' as $$ declare run_status text; begin
  if tg_table_name='ai_eval_runs' then
    if row(old.case_manifest,old.dataset_version,old.baseline_config_key,old.candidate_config_key,old.judge_config_key,old.maximum_case_count,old.cost_ceiling_usd,old.latency_ceiling_ms,old.initiated_by,old.request_id,old.created_at)
       is distinct from row(new.case_manifest,new.dataset_version,new.baseline_config_key,new.candidate_config_key,new.judge_config_key,new.maximum_case_count,new.cost_ceiling_usd,new.latency_ceiling_ms,new.initiated_by,new.request_id,new.created_at) then raise exception 'immutable_eval_run'; end if;
    if old.status in ('completed','completed_with_failures','failed','cancelled') and current_setting('app.ai_eval_run_resume',true)<>'rpc' then raise exception 'immutable_eval_run'; end if;
  else
...
language plpgsql security definer set search_path = '' as $$ declare manifest jsonb; existing public.ai_eval_runs; created public.ai_eval_runs; normalized jsonb; receipt jsonb; begin
  if _dataset_version<1 or _maximum_case_count not between 1 and 100 or _cost_ceiling_usd<=0 or _latency_ceiling_ms<=0 or greatest(char_length(_baseline_config_key),char_length(_candidate_config_key),char_length(_judge_config_key))>100 then raise exception 'invalid_request'; end if;
  normalized=jsonb_build_object('dataset_version',_dataset_version,'baseline_config_key',_baseline_config_key,'candidate_config_key',_candidate_config_key,'judge_config_key',_judge_config_key,'maximum_case_count',_maximum_case_count,'cost_ceiling_usd',_cost_ceiling_usd,'latency_ceiling_ms',_latency_ceiling_ms);
  ...
  if found then if existing.dataset_version is distinct from _dataset_version or existing.baseline_config_key is distinct from _baseline_config_key or existing.candidate_config_key is distinct from _candidate_config_key or existing.judge_config_key is distinct from _judge_config_key or existing.maximum_case_count is distinct from _maximum_case_count or existing.cost_ceiling_usd is distinct from _cost_ceiling_usd or existing.latency_ceiling_ms is distinct from _latency_ceiling_ms then raise exception 'request_id_collision'; end if; return jsonb_build_object('run_id',existing.id,'status',existing.status,'case_count',jsonb_array_length(existing.case_manifest)); end if;
  select jsonb_agg(jsonb_build_object('case_id',q.id,'revision',q.revision,'feature_key',q.feature_key) order by q.id,q.revision) into manifest from (select id,revision,feature_key from public.ai_eval_cases where status in ('approved','retired') and approved_in_dataset_version<=_dataset_version and (retired_in_dataset_version is null or retired_in_dataset_version>_dataset_version) order by id,revision limit _maximum_case_count) q;
  if manifest is null or jsonb_array_length(manifest)=0 then raise exception 'invalid_request'; end if;
  insert into public.ai_eval_runs(dataset_version,case_manifest,baseline_config_key,candidate_config_key,judge_config_key,maximum_case_count,cost_ceiling_usd,latency_ceiling_ms,initiated_by,request_id) values(_dataset_version,manifest,_baseline_config_key,_candidate_config_key,_judge_config_key,_maximum_case_count,_cost_ceiling_usd,_latency_ceiling_ms,_actor_id,_request_id) returning * into created;
...
create or replace function public.claim_ai_eval_run_batch(_run_id uuid,_batch_limit integer,_maximum_projected_cost_per_case numeric) returns jsonb
language plpgsql security definer set search_path = '' as $$ declare run public.ai_eval_runs; token uuid; cases jsonb; available integer; begin
  if _batch_limit<1 or _maximum_projected_cost_per_case<0 then raise exception 'invalid_request'; end if;
...
language plpgsql security definer set search_path = '' as $$ declare run public.ai_eval_runs; prior public.ai_eval_results; recorded public.ai_eval_results; manifest_case jsonb; begin
  ...
  on conflict(run_id,case_id,case_revision) do update set baseline_output=excluded.baseline_output,candidate_output=excluded.candidate_output,deterministic_assertions=excluded.deterministic_assertions,judge_verdict=excluded.judge_verdict,regression=excluded.regression,severe_regression=excluded.severe_regression,baseline_latency_ms=excluded.baseline_latency_ms,candidate_latency_ms=excluded.candidate_latency_ms,baseline_input_tokens=excluded.baseline_input_tokens,baseline_output_tokens=excluded.baseline_output_tokens,candidate_input_tokens=excluded.candidate_input_tokens,candidate_output_tokens=excluded.candidate_output_tokens,estimated_cost_usd=excluded.estimated_cost_usd,execution_status=excluded.execution_status,safe_failure_category=excluded.safe_failure_category,attempt_count=excluded.attempt_count,claim_token=excluded.claim_token,updated_at=now() where public.ai_eval_results.execution_status='failed'
...
language plpgsql security definer set search_path = '' as $$ declare run public.ai_eval_runs; normalized jsonb; prior jsonb; result jsonb; remaining boolean; begin
  ...
  else select exists(select 1 from jsonb_array_elements(run.case_manifest) m where not exists(select 1 from public.ai_eval_results r where r.run_id=run.id and r.case_id=(m->>'case_id')::uuid and r.case_revision=(m->>'revision')::integer and r.execution_status='succeeded')) into remaining; if run.status not in ('failed','completed_with_failures') or not remaining then raise exception 'state_conflict'; end if; perform set_config('app.ai_eval_run_resume','rpc',true); update public.ai_eval_runs set status='queued',completed_at=null,safe_failure_category=null,lease_token=null,lease_expires_at=null,updated_at=now() where id=_run_id; perform set_config('app.ai_eval_run_resume','',true); end if;
...
revoke all on function public.claim_ai_eval_run_batch(uuid,integer,numeric) from public,anon,authenticated;
...
grant execute on function public.claim_ai_eval_run_batch(uuid,integer,numeric) to service_role;
