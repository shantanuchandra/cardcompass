...
language plpgsql security definer set search_path = '' as $$ declare manifest jsonb; existing public.ai_eval_runs; created public.ai_eval_runs; normalized jsonb; receipt jsonb; _per_case_max_cost_usd numeric; begin
  ...
  select jsonb_agg(jsonb_build_object('case_id',q.id,'revision',q.revision,'feature_key',q.feature_key) order by q.source_feedback_id) into manifest from (select distinct on (source_feedback_id) id,source_feedback_id,revision,feature_key from public.ai_eval_cases where status in ('approved','retired') and approved_in_dataset_version<=_dataset_version and (retired_in_dataset_version is null or retired_in_dataset_version>_dataset_version) order by source_feedback_id,revision desc limit _maximum_case_count) q;
