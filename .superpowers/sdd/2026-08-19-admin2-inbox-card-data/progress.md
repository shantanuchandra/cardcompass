...
Ruling: Serialize each `(actor_id, request_id)` with a transaction-scoped advisory lock and persist a deterministic request fingerprint, returning the prior result only for an exact replay and raising `request_id_collision` for changed request semantics. Cost if wrong: request processing takes one additional transaction lock and older audit receipts without a fingerprint cannot be replayed through this new RPC.
