...
- REVIEW FIX FOLLOW-UP 2: the harness now builds an allowlisted child environment so hostile inherited libpq selectors cannot redirect the connection. It pins socket URLs to `/tmp:5432`, disables password/service files, rejects unsupported connection options, and preserves only explicitly supported URL-derived SSL/options values. Both explicit TCP and explicit socket isolated PostgreSQL runs passed 5/5 with no database or role residue.
