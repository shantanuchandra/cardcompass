...
- Flutter web selects path URL strategy at bootstrap and GoRouter uses base-relative locations so the browser exposes exactly one `/app/` prefix. The HTTP boundary serves safe extensionless `/app/*` navigation from the built index, redirects `/app` and the legacy catalog path canonically, and never turns missing assets, API-like paths, encoded traversal, or lookalike `admin2.js` into successful SPA responses. Hash routes are legacy input only and are not claimed as PathUrlStrategy navigation.
...
- Compiled-browser acceptance uses `tool/browser_fixture_main.dart`, an explicit test-only target with in-memory auth/admin/data providers and forced semantics. Each direct navigation requires a new CDP loader and three stable destination-specific semantic samples. The production `lib/main.dart` release build was scanned before the fixture build and contains no fixture marker, fake-auth query, or fixture entrypoint string.
