...
The compiled-web follow-up corrected GoRouter to use base-relative internal locations under Flutter's `/app/` base href, preventing `/app/app/...` browser URLs. A real release build in headless Chrome rendered direct login, Admin2, Cards, Settings, and legacy-entry navigation with one base prefix. Static-host rewrites are exact route families rather than a broad `admin2*` mask, and hash compatibility is no longer claimed under PathUrlStrategy.
