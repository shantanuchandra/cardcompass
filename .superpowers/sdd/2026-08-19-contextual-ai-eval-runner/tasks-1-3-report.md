...
Scoped re-review correction: latest-revision ranking precedes retirement exclusion, candidate keys derive an exact manifest feature family, empty matching datasets fail safely, and only the public proxy—not typed transport callers—may resolve an omitted model to the supported default.
