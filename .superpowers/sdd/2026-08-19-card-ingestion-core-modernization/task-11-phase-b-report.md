...
| 6. Quarantine episodes           | The producer fence persists a monotonic integer episode and semantic identity. Same-occurrence concurrency shares one review dedupe identity; admin Retry leaves the prior terminal review/audit immutable, and the next failure advances the episode and stages exactly one new pending review. A terminal pending item from a pre-episode worker remains a single item: SQL recognizes its exact persisted legacy fence/source shape as derived episode one, while new occurrences require the versioned identity.                                                                                     | Behavioral Retry → re-quarantine concurrency, terminal legacy single-item, same-episode replay, and Task 7 full-fence binding.                                                                               |
...
- `index_test.ts` + catalog publication + issuer crawl: **206/206**.
- Cross-slice shared + batch + supporting integration: **208/208**.
...
Yes**, with no remaining Critical, Important, or Minor findings.
...
identity without an episode field is accepted only as derived episode one when
both the persisted fence and pending review retain the exact legacy shape;
episode two and later require the new form.
