	-- CardCompass - Fix Schema to Replace credit_cards with card_catalog
-- Generated on: July 3, 2025

-- Replace references to credit_cards with card_catalog
...
-- 2. Migrate data from credit_cards to card_catalog (if needed)
INSERT INTO card_catalog (id, card_name, bank_name, network, card_type, annual_fee, is_active, created_at)
SELECT id, card_name, bank_name, network, card_type, annual_fee, TRUE, created_at
FROM credit_cards
WHERE NOT EXISTS (SELECT 1 FROM card_catalog WHERE card_catalog.id = credit_cards.id)
...
DO $$
BEGIN
  RAISE NOTICE 'Schema update completed: credit_cards table references replaced with card_catalog';
END $$;
