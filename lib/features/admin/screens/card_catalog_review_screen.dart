...
class _CardCatalogReviewScreenState extends State<CardCatalogReviewScreen> {
  ...
  Future<void> _editAndApprove(Map<String, dynamic> item) async {
    ...
    final network = TextEditingController(text: original['network']?.toString());
        ...
        content: SizedBox(
          ...
          child: Column(
            ...
            children: [
              ...
              TextField(controller: name, decoration: const InputDecoration(labelText: 'Card name')),
              TextField(controller: network, decoration: const InputDecoration(labelText: 'Network')),
            ],
          ),
        ),
        actions: [
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          if (items.isEmpty) {
            return Center(child: Text('No $_status catalog items.'));
          }
          return ListView.separated(
            ...
            itemBuilder: (context, index) => _ReviewCard(
              item: items[index],
              onApprove: () => _act('approve', items[index]),
              onEditApprove: () => _editAndApprove(items[index]),
              onMerge: (cardId) => _act(
                'merge',
                items[index],
                mergeCardId: cardId,
              ...
              onRetry: () async {
                final note = await _askReason('Retry official-source discovery');
                if (note != null) await _act('retry', items[index], reason: note);
              },
              onReject: () async {
                final reason = await _askReason('Reject catalog proposal', required: true);
                if (reason != null) await _act('reject', items[index], reason: reason);
              },
            ),
          );
        },
...
class _ReviewCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final fields = Map<String, dynamic>.from(item['proposed_fields'] as Map? ?? const {});
    final job = Map<String, dynamic>.from(item['card_discovery_jobs'] as Map? ?? const {});
    final evidence = Map<String, dynamic>.from(job['evidence'] as Map? ?? const {});
            ...
            if (pending) ...[
              ...
              Wrap(
                ...
                children: [
                  ...
                  OutlinedButton(onPressed: onEditApprove, child: const Text('Edit & approve')),
                  for (final candidate in candidates.whereType<Map>())
                    OutlinedButton(
                      onPressed: candidate['id'] is String
                          ? () => onMerge(candidate['id'] as String)
                          : null,
                      child: Text('Merge: ${candidate['card_name'] ?? 'existing'}'),
                    ),
                  TextButton(onPressed: onRetry, child: const Text('Retry scrape')),
