...
class SupabaseBenefitsRepository {
  ...
  Future<List<Benefit>> getAllBenefits() async {
    if (_allBenefitsCache != null) {
      print(
          '💾 SupabaseBenefitsRepository: Returning cached benefits (${_allBenefitsCache!.length} benefits)');
    ...
    try {
      ...
      final benefits =
          asList(response).map((json) => Benefit.fromJson(json)).toList();

      _allBenefitsCache = benefits;
      print(
          '💾 SupabaseBenefitsRepository: Cached benefits (${benefits.length} benefits)');
      return benefits;
    } catch (e) {
  ...
  Future<List<Benefit>> getBenefitsByCategories(List<String> categories) async {
    try {
      ...
      return asList(response).map((json) => Benefit.fromJson(json)).toList();
    } catch (e) {
  ...
  Future<List<CardBenefit>> getCardBenefits(String cardId) async {
    try {
      ...
      return asList(response)
          .map((json) => CardBenefit.fromMappingJson(json))
          .toList();
    } catch (e) {
  ...
  Future<List<CardBenefit>> getUserCardBenefits(String userId) async {
    if (_userCardBenefitsCache.containsKey(userId)) {
      print(
          '💾 SupabaseBenefitsRepository: Returning cached user card benefits for user: $userId (${_userCardBenefitsCache[userId]!.length} benefits)');
      return _userCardBenefitsCache[userId]!;
    }

    try {
      final response = await _supabase.from('user_cards').select('''
            *,
            card_catalog!inner(
              id,
              bank,
              card_name,
              card_benefit_mapping!inner(
                *,
                benefits!inner(*)
              )
            )
      ...
      for (final userCardJson in asList(response)) {
        final cardBenefits =
            asListDynamic(userCardJson['card_catalog']['card_benefit_mapping']);
        for (final benefitJson in cardBenefits) {
          allBenefits.add(CardBenefit.fromMappingJson(benefitJson));
        }
      }

      _userCardBenefitsCache[userId] = allBenefits;
      print(
          '💾 SupabaseBenefitsRepository: Cached user card benefits for user: $userId (${allBenefits.length} benefits)');
      return allBenefits;
    } catch (e) {
  ...
  Future<void> deleteBenefit(String benefitId) async {
    try {
      await _supabase
          .from('benefits')
          .update({'is_active': false}).eq('id', benefitId);
    } catch (e) {
  ...
  Future<void> trackBenefitUsage(String benefitId, String userId) async {
    try {
      await _supabase.from('benefit_usage').insert({
