...
class CardDiscoveryService {
  ...
  Future<String?> discoverAndAddCard({
    ...
    try {
      ...
      if (exactMatch != null) {
        print('✅ Found exact match: ${exactMatch['card_name']} (${exactMatch['bank']})');
      ...
      if (similarCards.isNotEmpty) {
        ...
        print('   💡 If yes, update your detection logic to use the exact name');
      }
      ...
      if (existingCard != null) {
        print('✅ Card already exists with this URL: ${existingCard['card_name']}');
  ...
  Future<Map<String, dynamic>?> _findExactCardMatch(String bankName, String cardName) async {
    try {
  ...
  Future<List<Map<String, dynamic>>> _findSimilarCards(String bankName, String cardName) async {
    try {
      final response = await _supabase
          .from('card_catalog')
          .select('*')
  ...
  Future<String?> _searchForProductUrl(String bankName, String cardName) async {
    try {
      ...
      final searchQuery = Uri.encodeComponent('$bankName $cardName credit card official');
      final searchUrl = 'https://www.google.com/search?q=$searchQuery';
      
      print('   🔍 Searching Google: "$bankName $cardName credit card official"');
  ...
  Future<List<Map<String, dynamic>>> _extractBenefitsFromUrl(String url, String bankName) async {
    try {
      ...
      print('   ⚠️  Using fallback: Benefit extraction deferred to full import service');
      print('   💡 Run the robust benefit import to populate benefits for this card');
  ...
  List<String> _generateUrlPatterns(String bankName, String cardName) {
    ...
    if (bankName.toLowerCase().contains('hdfc')) {
      patterns.add('https://www.hdfcbank.com/personal/pay/cards/credit-cards/$cardSlug');
    }
    
    // ICICI Bank patterns
    if (bankName.toLowerCase().contains('icici')) {
      patterns.add('https://www.icicibank.com/personal-banking/cards/credit-card/$cardSlug');
    }
    
    // Axis Bank patterns
    if (bankName.toLowerCase().contains('axis')) {
      patterns.add('https://www.axisbank.com/retail/cards/credit-card/$cardSlug');
    }
    
    // SBI Card patterns
    if (bankName.toLowerCase().contains('sbi')) {
      patterns.add('https://www.sbicard.com/en/personal/credit-cards/$cardSlug');
    }
    ...
    if (bankName.toLowerCase().contains('kotak')) {
      patterns.add('https://www.kotak.com/en/personal-banking/cards/credit-cards/$cardSlug.html');
    }
    
    // Punjab National Bank patterns
    if (bankName.toLowerCase().contains('punjab') || bankName.toLowerCase().contains('pnb')) {
  ...
  Future<String?> _createNewCard({
    ...
    try {
      // Create card via RPC (card_url will be null initially)
      await _supabase.rpc('create_or_get_card_catalog', params: {
        '_bank': bankName,
        '_card_name': cardName,
        '_network': 'visa', // Default - should be detected
        '_card_type': 'credit',
        '_annual_fee': 999.0, // Default - should be extracted from benefits
        '_apr': 3.5, // Default APR
        '_joining_fee': 0.0, // Default joining fee
      });
      
      // Find the newly created card by bank and card name
      ...
      final cardId = newCard['id'] as String;
  ...
  Future<void> _importBenefitsForCard(String cardUrl, String cardId, String bankName) async {
    try {
      ...
      if (benefits.isEmpty) {
        print('   ⚠️  No benefits extracted - will be populated by full import service');
        return;
      }
