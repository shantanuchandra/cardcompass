...
class BenefitImportService {
  ...
  /// Import benefits from the enhanced_credit_cards.csv file
    ...
    try {
      ...
      if (!tablesExist) {
        return {
          'success': false,
          'message':
              'Database tables missing. Please run database setup first.',
          'error':
              'Required tables (benefit_categories, benefits, card_benefit_mapping) do not exist',
          'cards_processed': 0,
          'benefits_created': 0,
          'total_csv_records': 0,
        };
      ...
      try {
        ...
        csvData =
            await rootBundle.loadString('assets/enhanced_credit_cards.csv');
        ...
        if (await file.exists()) {
          ...
          throw Exception(
              'enhanced_credit_cards.csv not found in assets or project root');
        }
      }
      ...
      print(
          '📝 CSV first 200 characters: ${csvData.substring(0, csvData.length > 200 ? 200 : csvData.length)}');
      ...
      for (var row in rows) {
        ...
        final rawBankName = rowMap['Bank']?.toString() ?? '';
        if (rawCardName.isNotEmpty && rawBankName.isNotEmpty) {
          // Apply name normalization
          final normalizedBankName =
              GeminiTransactionParser.normalizeBankName(rawBankName);
          final cardVariantName = GeminiTransactionParser.normalizeCardName(
              rawCardName, normalizedBankName);

          print(
              '🔄 Processing: $rawCardName ($rawBankName) -> $cardVariantName ($normalizedBankName)');

          // First try to find matching card in database
          final matchingCards = existingCards
              .where((card) =>
                  card['card_name']
                      .toString()
                      .toLowerCase()
                      .contains(cardVariantName.toLowerCase()) ||
                  cardVariantName
                      .toLowerCase()
                      .contains(card['card_name'].toString().toLowerCase()) ||
                  card['card_name']
                      .toString()
                      .toLowerCase()
                      .contains(rawCardName.toLowerCase()) ||
                  rawCardName
                      .toLowerCase()
                      .contains(card['card_name'].toString().toLowerCase()))
              .toList();
          ...
          if (matchingCards.isNotEmpty) {
            ...
            print(
                '🆕 Creating new card: $cardVariantName ($normalizedBankName)');
            final newCard =
                await _createCard(cardVariantName, normalizedBankName);
            if (newCard != null) {
              cardId = newCard['id'];
          ...
          final benefitsCount = await _createCardBenefits(
              cardId, cardVariantName, normalizedBankName);
    ...
    final categories = [
      {
        'code': 'DINING',
        'name': 'Dining & Food',
        'description': 'Restaurants, food delivery, and dining'
      },
      {
        'code': 'TRAVEL',
        'name': 'Travel & Transportation',
        'description': 'Flights, hotels, cab rides, and travel'
      },
      {
        'code': 'FUEL',
        'name': 'Fuel & Petrol',
      ...
      try {
        ...
        if (existing == null) {
          await _supabase.from('benefits').insert({
            ...
            'description':
                'Earn ${benefitData['value']}% on ${benefitData['category']?.toString().toLowerCase()} purchases',
  ...
  Future<List<Map<String, dynamic>>> _getAllCardsFromDatabase() async {
    try {
      ...
      final catalogResponse =
          await _supabase.from('card_catalog').select('id, card_name, bank');
  ...
  Future<int> _createCardBenefits(
      String cardId, String cardName, String bankName) async {
    ...
    for (final mapping in benefitMappings) {
      try {
        ...
        final benefitResponse = await _supabase
            .from('benefits')
            .select('benefit_id')
            .eq('title', mapping['benefit_name'])
            .maybeSingle();

        if (benefitResponse != null) {
          final benefitId = benefitResponse['benefit_id'];
          ...
          if (existingCardBenefit == null) {
            await _supabase.from('card_benefit_mapping').insert({
              'card_id': cardId,
            ...
            benefitsCreated++;
            print(
                '✅ Created card benefit: ${mapping['benefit_name']} for $cardName');
          }
        }
      } catch (e) {
  ...
  List<Map<String, dynamic>> _getBenefitMappingsForCard(
    ...
    // Dining benefits
    if (cardLower.contains('dining') ||
        cardLower.contains('food') ||
        cardLower.contains('zomato') ||
        cardLower.contains('swiggy')) {
      mappings.add({
        'category': 'DINING',
    ...
    // Travel benefits
    if (cardLower.contains('travel') ||
        cardLower.contains('miles') ||
        cardLower.contains('vistara') ||
        cardLower.contains('air')) {
      mappings.addAll([
        {
          'category': 'TRAVEL',
    ...
    // Fuel benefits
    if (cardLower.contains('fuel') ||
        cardLower.contains('petrol') ||
        cardLower.contains('hpcl') ||
        cardLower.contains('iocl')) {
      mappings.add({
        'category': 'FUEL',
    ...
    // Shopping benefits
    if (cardLower.contains('shopping') ||
        cardLower.contains('amazon') ||
        cardLower.contains('flipkart') ||
        cardLower.contains('e-commerce')) {
      mappings.add({
        'category': 'SHOPPING',
    ...
    if (cardLower.contains('premium') ||
        cardLower.contains('infinite') ||
        cardLower.contains('signature') ||
        cardLower.contains('elite') ||
        cardLower.contains('platinum') ||
        cardLower.contains('black')) {
      mappings.add({
        'category': 'ENTERTAINMENT',
  ...
  Future<void> _ensureDatabaseTables() async {
    try {
      ...
      if (!tablesExist) {
        throw Exception(
            'Required database tables missing. Please run the setup_benefit_tables.sql script in Supabase first.');
      }
  ...
  Future<bool> _checkTablesExist() async {
    try {
      ...
      await _supabase
          ...
          .select('category_code')
          .limit(1);
      await _supabase.from('benefits').select('benefit_id').limit(1);
      await _supabase
          .from('card_benefit_mapping')
          .select('mapping_id')
          .limit(1);
  ...
  Future<Map<String, dynamic>?> _createCard(
      String cardName, String bankName) async {
    try {
