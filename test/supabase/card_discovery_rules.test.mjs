...
test("extracts a product identity from legacy issuer page headings", () => {
  ...
  assert.deepEqual(officialCardIdentityFromHtml(html, "Punjab National Bank"), {
...
test("normal terms boilerplate is context rather than a competing card identity", () => {
  for (
    const prose of [
      "Fees and charges for Privilege Credit Card",
      "Terms and conditions for Privilege Credit Card",
      "Benefits of Privilege Credit Card",
    ]
  ) {
    const html =
      `<title>Privilege Credit Card | Axis Bank</title><p>${prose}</p>`;
    assert.equal(
      exactOfficialPageIdentity(html, "Axis Bank", "Privilege")?.cardName,
      "Privilege",
      prose,
    );
  }
...
test("authenticated discovery retains publication evidence for central admin review", async () => {
  const source = await readFile(cardDiscoveryEntrypoint, "utf8");
  assert.match(source, /authenticated_source_requires_admin_review/);
  assert.match(source, /publicationFieldsFromFetch\(\{/);
  assert.match(source, /sourceIdentityHash:\s*submittedHash/);
  assert.match(source, /finalResourceIdentityHash:\s*finalHash/);
  assert.match(source, /finalResourceUrl\s*\?\?\s*page\.finalUrl/);
  assert.doesNotMatch(source, /enqueueBenefitEnrichmentJob\(db,/);
  assert.doesNotMatch(source, /from\(["']card_catalog["']\)\s*\.insert\(/);
  assert.doesNotMatch(source, /discovery_job_id:\s*job\.id,[\s\S]{0,160}alias/);
  assert.doesNotMatch(
    source,
    /onConflict:\s*["']card_id,final_url_hash,content_hash["']/,
  );
...
test("clears stale URL failures when a discovery job enters review", () => {
  assert.deepEqual(
    reviewRequiredJobPatch("review-1", "2026-08-17T00:00:00.000Z"),
    {
...
test("maps internal URL failures to stable public reason codes", () => {
  assert.equal(
    publicReasonCode(new Error("unapproved_domain")),
    "unapproved_domain",
  );
  assert.equal(
    publicReasonCode(new DOMException("timed out", "TimeoutError")),
    "fetch_timeout",
  );
  assert.equal(
    publicReasonCode(new Error("official_fetch_503")),
    "fetch_timeout",
  );
  assert.equal(
    publicReasonCode(new Error("database connection detail")),
    "review_required",
  );
});
...
test("requires official issuer evidence plus an agreeing statement signal", () => {
  ...
  assert.ok(
    missingStatementSignal.reasons.includes("missing_statement_signal"),
  );
});
...
test("card discovery gives every official fetch an invocation deadline", async () => {
  const source = await readFile(cardDiscoveryEntrypoint, "utf8");
  const calls = [
    ...source.matchAll(/fetchOfficialIssuerResource\(\{([\s\S]*?)\n\s*\}\)/g),
  ];
