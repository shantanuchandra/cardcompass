...
class _FakeDataSource implements MovieDealsDataSource {
  ...
  Future<List<Map<String, dynamic>>> loadCatalogCards(
    List<String> cardIds,
  ) async => catalogCards.where((c) => cardIds.contains(c['id'])).toList();
...
void main() {
  test(
    'Supabase movie reads use the database active mapping view for benefits and mappings',
    () async {
      ...
      final source = SupabaseMovieDealsDataSource(client);

      await source.loadMovieRelatedBenefits();
      await source.loadMappings(['benefit-1']);

      expect(requests, hasLength(2));
      expect(requests.map((request) => request.uri.path).toSet(), {
        '/rest/v1/active_card_benefits',
      });
      expect(
        requests.expand((request) => request.uri.queryParameters.keys),
        isNot(contains('is_active')),
      );
    },
  );
  ...
  group('MovieDealsSupabaseRepository — source construction', () {
    test(
      'owned status is matched via catalog_card_id, not the user_card row id',
      () {
    ...
    test(
      'partners and excludedCategories are extracted from their own columns, not value_config',
      () {
        final dataSource = _FakeDataSource(
          benefits: [
            _benefitRow(
    ...
    test(
      'validityStart/validityEnd are extracted from valid_from/valid_until columns',
      () {
  ...
  group('MovieDealsSupabaseRepository — context building', () {
    test(
      'confirmations are scoped per (catalogCardId, benefitId), never unioned across benefits on the same card',
      () {
        final dataSource = _FakeDataSource(
          benefits: [
            _benefitRow(
        ...
        return repository
            ...
            .then((snapshot) {
              expect(
                snapshot.contexts[('catalog-card-1', 'b1')]?.confirmedPlatforms,
                contains('pvr'),
              );
              expect(
                snapshot
                        .contexts[('catalog-card-1', 'b2')]
                        ?.confirmedPlatforms ??
                    const {},
                isNot(contains('pvr')),
              );
    ...
    test(
      'capped usage is unverified when matching transaction metadata lacks a numeric ticket_count',
      () async {
        final dataSource = _FakeDataSource(
          benefits: [
            _benefitRow(
        ...
        expect(
          snapshot.contexts[('catalog-card-1', 'b1')]?.usageConfidence,
    ...
    test(
      'milestone spend selects the most recently COMPLETED cycle, not the most recently UPDATED row',
      () async {
        ...
        expect(
          snapshot.contexts[('catalog-card-1', 'b1')]?.milestoneSpend,
          85000.0,
        );
      },
    );
    ...
    test(
      'absent milestone cache leaves milestoneSpend null, not zero',
      () async {
        final dataSource = _FakeDataSource(
          benefits: [
            _benefitRow(
    ...
    test(
      'confirmPlatform delegates to the data source with the given ids',
      () async {
        final dataSource = _FakeDataSource();
        final repository = MovieDealsSupabaseRepository(dataSource);
    ...
    test(
      'confirmationCount is read from the confirmation row and threaded onto the context',
      () async {
        ...
        expect(
          snapshot.contexts[('catalog-card-1', 'b1')]?.confirmationCount,
          12,
        );
      },
    );
    ...
    test(
      'a benefit with no confirmation row has a null confirmationCount, never a fabricated 0',
      () async {
        final dataSource = _FakeDataSource(
          benefits: [
            _benefitRow(
    ...
    test(
      'a confirmation row keyed by platform_key (the real view\'s column) still resolves to a non-empty confirmedPlatforms',
      () async {
